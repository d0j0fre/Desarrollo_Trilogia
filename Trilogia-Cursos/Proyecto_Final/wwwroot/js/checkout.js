(function () {
    ...
    ready(function () {
        var provincia = getElement("provincia");
        var canton = getElement("canton");
        var distrito = getElement("distrito");

        if (!provincia || !canton || !distrito) {
            return;
        }

        var savedProvincia = getSavedValue("savedProvincia");
        var savedCanton = getSavedValue("savedCanton");
        var savedDistrito = getSavedValue("savedDistrito");

        function cargarProvincias(selectedValue) {
            clearOptions(provincia, "Elige una opción...");
            addOptions(provincia, Object.keys(ubicaciones));
            ensureOption(provincia, selectedValue);
            provincia.value = selectedValue || "";
        }

        function cargarCantones(selectedValue) {
            var provinciaSeleccionada = provincia.value;
            var cantones = provinciaSeleccionada && ubicaciones[provinciaSeleccionada]
                ? Object.keys(ubicaciones[provinciaSeleccionada])
                : [];

            clearOptions(canton, "Seleccione una opción");
            addOptions(canton, cantones);
            ensureOption(canton, selectedValue);
            canton.disabled = cantones.length === 0;
            canton.value = selectedValue || "";
        }

        function cargarDistritos(selectedValue) {
            var provinciaSeleccionada = provincia.value;
            var cantonSeleccionado = canton.value;
            var distritos = provinciaSeleccionada
                && cantonSeleccionado
                && ubicaciones[provinciaSeleccionada]
                && ubicaciones[provinciaSeleccionada][cantonSeleccionado]
                    ? ubicaciones[provinciaSeleccionada][cantonSeleccionado]
                    : [];

            clearOptions(distrito, "Seleccione una opción");
            addOptions(distrito, distritos);
            ensureOption(distrito, selectedValue);
            distrito.disabled = distritos.length === 0;
            distrito.value = selectedValue || "";
        }

        cargarProvincias(savedProvincia);
        cargarCantones(savedCanton);
        cargarDistritos(savedDistrito);
