...
-- Limpieza: la tabla PlanillaDetalle vieja (no relacionada, confirmada vacía) sigue existiendo por el rollback de 0014
IF OBJECT_ID(N'dbo.PlanillaDetalle', N'U') IS NOT NULL AND COL_LENGTH('dbo.PlanillaDetalle', 'PlanillaDetalleId') IS NULL
BEGIN
...
-- Cada tabla en su propia transacción, para que un fallo posterior no revierta las anteriores
...
-- Corrige el ledger: 0013 y 0014 quedaron mal registradas como 'Applied' pese a fallar parcialmente
UPDATE dbo.SchemaMigrationHistory
SET Status = 'Failed', Notes = Notes + ' [Corregido: fallo en cascada por XACT_ABORT sobre transacción compartida; ver 0015]'
WHERE MigrationId IN ('0013_rrhh_planilla', '0014_rrhh_planilla_fix');

INSERT INTO dbo.SchemaMigrationHistory
    (MigrationId, FileName, FileSha256, Status, AppliedBy, EnvironmentName, Notes)
VALUES
    ('0015_rrhh_planilla_fix2', 'database/migrations/0015_rrhh_planilla_fix2.sql', 'PENDIENTE_CALCULAR_SHA256',
     'Applied', SUSER_SNAME(), 'DEV', 'Crea correctamente las 5 tablas de planilla (transacciones aisladas por tabla) y recrea los procedimientos.');
GO

PRINT 'Migración 0015_rrhh_planilla_fix2 aplicada correctamente.';
GO